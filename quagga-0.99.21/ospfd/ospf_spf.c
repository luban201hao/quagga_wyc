...
int nexthop_count,parent_count,vertex_count,enqueue_count;
	       ...
	       struct pqueue * candidate)
{
  ...
  while (p < lim)
  {
      ...
      //section2:if w_lsa exist, new a vertex from w_lsa 
...
ospf_spf_calculate_timer (struct thread *thread)
{
  ...
  zlog_debug ("spftest: calculation begin %ld.%06ld",help_time.tv_sec,help_time.tv_usec);
  if (IS_DEBUG_OSPF_EVENT)
  ...
  quagga_gettime (QUAGGA_CLK_MONOTONIC,&help_time);
  zlog_debug ("spftest: all calculation complete %ld.%06ld",help_time.tv_sec,help_time.tv_usec);
  return 0;
}
