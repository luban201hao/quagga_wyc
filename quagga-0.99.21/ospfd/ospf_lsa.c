...
/* Duplicate OSPF LSA. */
//lsa->lsdb is not specify in this func
struct ospf_lsa *
ospf_lsa_dup (struct ospf_lsa *lsa)
{
  ...
  {
      new->phase_matrix=(int **)calloc(new->links_count,sizeof(int *));
      for(int j=0;j<new->links_count;++j)
      {
        new->phase_matrix[j]=(int *)calloc(new->phase_count,sizeof(int));
      }
      for(int i=0;i<new->links_count;++i)
...
ospf_lsa_free (struct ospf_lsa *lsa)
{
  ...
  {
    //zlog_debug("lsa->phase_count=%d,lsa->link_count=%d",lsa->phase_count,lsa->links_count);
    
    if(lsa->phase_matrix!=NULL)
    {
      for(int j=0;j<lsa->links_count;++j)
      {
        //zlog_debug("j=%d",j);
        if(lsa->phase_matrix[j]!=NULL)
        {
          free(lsa->phase_matrix[j]);
          lsa->phase_matrix[j]=NULL;
        }

      };
      free(lsa->phase_matrix);
      lsa->phase_matrix=NULL;
    }
  }
  //zlog_debug("in ospf_lsa_free,free phase_matrix success");
  /* Delete LSA data. */
  if (lsa->data != NULL)
...
static int
lsa_link_se_set (struct stream *s, struct ospf_interface *oi)
{
  int links = 0;
  struct ospf_neighbor *nbr;
  struct in_addr id, mask;
  ...
  zlog_debug("in func lsa_link_se_set,id=%x,mask=%x",id.s_addr,mask.s_addr);
  
  links += link_info_set (s, id, mask, LSA_LINK_TYPE_SE, 0, oi->output_cost);
  return links;
}
...
{
  //zlog_debug("in func ospf_router_lsa_new");
  //zlog_debug("area->area_id=%x",area->area_id.s_addr);
  //zlog_debug("ospf->router_id=%x",area->ospf->router_id.s_addr);
...
ospf_router_lsa_refresh (struct ospf_lsa *lsa)
{
  ...
  //zlog_debug("in func ospf_router_lsa_refresh,new_lsa_seq=%x",new->data->ls_seqnum);
  //old lsa will be discard here
...
ospf_router_lsa_update_area (struct ospf_area *area)
{
  ...
  //zlog_debug ("in func ospf_router_lsa_update_area");
