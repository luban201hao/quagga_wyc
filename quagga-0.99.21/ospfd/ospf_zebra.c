                            ...
                            zebra_size_t length)
{
  ...
  struct ospf_interface *oi;
  //look for se oi and update oi->connected
  if(prefix_match(se_prefix,c->address))
  {
    for(rn=route_top(IF_OIFS(c->ifp));rn;rn=route_next(rn))
    {
      if(oi=(struct ospf_interface *)rn->info)
      {
