...
ospf_new (void)
{
  ...
  new->se_p_now=prefix_new();
  new->se_p_old=prefix_new ();
  new->se_p_now->u.prefix4.s_addr=0;
  new->se_p_now->prefixlen=0;
  new->se_p_old->u.prefix4.s_addr=0;
  new->se_p_old->prefixlen=0;
  ...
  for (ALL_LIST_ELEMENTS_RO (ifp->connected, cnode, co))
    {
      ...
      if (CHECK_FLAG(co->flags,ZEBRA_IFA_SECONDARY))
        continue;
      //if co is ipv4 and co is not in ipf->info->oifs and co match p,new a oi. So connect info is added automatically 
      //but se is in ospf->oi list, so this do not happen
