...
main (int argc, char **argv)
{
  ...
  while (1) 
    {
      ...
      opt = getopt_long (argc, argv, "df:i:z:hA:P:u:g:avCq:", longopts, 0);
  ...
  neighbor_info_list_cur=neighbor_info_list_new();
  ase_info_list_cur=(struct ase_info_list *)calloc(1,sizeof(struct ase_info_list));
  se_info_list_cur=se_info_list_new();

  se_ase_info_list_cur=se_ase_info_list_new();
  ...
  /* Create VTY socket */  //modity socket loction
  if(vtypath==NULL){
